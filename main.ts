  ...
  <div class="app-shell">
    ...
    <main>
      ...
      <section id="lessons" class="screen" aria-labelledby="lessonName">
        ...
        <div id="lessonNumbers" aria-label="Kies een les" style="display:flex;gap:6px;justify-content:center;flex-wrap:wrap;margin:10px 0 2px;"></div>
        <div style="display:flex;gap:10px;margin:8px 0 2px;">
          <button id="previousBottom" type="button" style="flex:1;min-height:44px;border:1px solid var(--line-soft);border-radius:12px;background:rgba(var(--surface-rgb),.55);color:var(--text);font-weight:700;">‹ Vorige</button>
          <button id="nextBottom" type="button" style="flex:1;min-height:44px;border:1px solid var(--line-soft);border-radius:12px;background:rgba(var(--surface-rgb),.55);color:var(--text);font-weight:700;">Volgende ›</button>
        </div>
...
const renderLessonNumbers = () => {
  const box = get<HTMLDivElement>("lessonNumbers");
  box.innerHTML = lessons.map((lesson, index) => {
    const active = index === lessonIndex;
    return `<button type="button" data-lesson-index="${index}" aria-label="Les ${index + 1}: ${lesson.name}"
      style="min-width:40px;height:40px;border:1px solid ${active ? "var(--cyan)" : "var(--line-soft)"};border-radius:10px;background:${active ? "var(--cyan)" : "rgba(var(--surface-rgb),.55)"};color:${active ? "var(--bg-deep)" : "var(--text)"};font-weight:800;">
      ${index + 1}
    </button>`;
  }).join("");

  box.querySelectorAll<HTMLButtonElement>("[data-lesson-index]").forEach((button) => {
    button.addEventListener("click", () => {
      loadLesson(Number(button.dataset.lessonIndex));
    });
  });
};
...
const loadLesson = (index: number, autoplay = false) => {
  ...
  // Eerst lokale audio (public/audio), daarna automatisch CDN als lokaal ontbreekt.
  const localUrl = `/audio/${lesson.id}.mp3`;
  const cdnUrl = `https://cdn.islamic.network/quran/audio-surah/128/ar.alafasy/${lesson.id}.mp3`;
  ...
  let triedCdn = false;
  const tryCdnIfLocalFails = () => {
    if (triedCdn) return;
    triedCdn = true;
    audio.src = cdnUrl;
    audio.load();
    console.log("Lokale audio niet gevonden, CDN gebruikt:", cdnUrl);
  };
  audio.onerror = tryCdnIfLocalFails;
  audio.src = localUrl;
  audio.load();
...
get<HTMLButtonElement>("previousBottom").addEventListener("click", () => loadLesson(lessonIndex - 1));
get<HTMLButtonElement>("nextBottom").addEventListener("click", () => loadLesson(lessonIndex + 1));
...
// Zorg dat de vorige/volgende pijlen ook op iPhone/iPad zichtbaar zijn.
const previousArrow = get<HTMLButtonElement>("previous");
const nextArrow = get<HTMLButtonElement>("next");
